...
public class ManagerController {
        ...
        @PostMapping
        @Operation(summary = "Cadastrar gestor", description = "Cadastra um gestor no banco de dados")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "201", description = "Gestor cadastrado com sucesso"),
                        @ApiResponse(responseCode = "400", description = "Dados inválidos"),
                        @ApiResponse(responseCode = "500", description = "Erro interno no servidor")
        })
        public ResponseEntity<ManagerResponseDTO> create(@AuthenticationPrincipal User user,
                        @Valid @RequestBody ManagerRequestDTO managerRequestDTO) {
                ManagerResponseDTO manager = managerService.create(user, managerRequestDTO);
                return ResponseEntity.ok(manager);
        }

        @GetMapping
        @Operation(summary = "Listar Gestores", description = "Retorna uma lista de Gestores, ordenada por nome (10 por página)")
        @ApiResponses(value = {
        ...
        @GetMapping("/{id}")
        @Operation(summary = "Obter Gestor por ID", description = "Retorna os dados de um gestor com base no ID informado.")
        @ApiResponses(value = {
        ...
        @DeleteMapping("/{id}")
        @Operation(summary = "Remover gestor", description = "Remove definitivamente um gestor e seu usuário associado.")
        @ApiResponses(value = {
        ...
        @Operation(summary = "Reatribuir gestor para outra loja", description = "Atualiza o dealer associado ao gestor.")
        @ApiResponses(value = {
        ...
        @PutMapping("/{id}")
        @Operation(summary = "Atualizar Gestor por ID", description = "Atualiza os dados de um gestor com base no ID informado.")
        @ApiResponses(value = {
        ...
        public ResponseEntity<ManagerResponseDTO> update(@PathVariable Long id, @AuthenticationPrincipal User user,
                        @Valid @RequestBody ManagerRequestDTO managerRequestDTO) {
                ManagerResponseDTO manager = managerService.update(user, id, managerRequestDTO);
                return ResponseEntity.ok().body(manager);
        }
}
