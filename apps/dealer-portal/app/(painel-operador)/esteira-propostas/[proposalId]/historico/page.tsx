...
const extractSellerName = (metadata: Record<string, unknown> | null) => {
  ...
  for (const candidate of candidates) {
    ...
    if (candidate && typeof candidate === "object") {
      ...
      const nested = record.name ?? record.fullName ?? record.nome ?? record.email;
  ...
  useEffect(() => {
    const loadNames = async () => {
      try {
        ...
        const dealerMap = dealers.reduce<Record<number, { name: string; enterprise?: string }>>(
          (acc, dealer) => {
            if (dealer.id) {
              const name =
                dealer.fullName ??
        ...
        console.warn(
          "[Operador Historico] Nao foi possivel carregar nomes de lojas/vendedores",
          err,
        );
      }
    };
  ...
  const performStatusUpdate = useCallback(
    async (nextStatus: ProposalStatus, contractNumber?: string) => {
      ...
      try {
        ...
        messageApi.success(
          `Status atualizado: a ficha agora esta ${statusLabel[nextStatus] ?? nextStatus}.`,
        );
                ...
                <Col xs={24} lg={12}>
                  <Card title="Endereco e renda" className="shadow-sm">
                    <Descriptions column={2} size="small" styles={{ label: labelStyle }}>
                      ...
                      <Descriptions.Item label="Renda" span={2}>
