...
export default function ProposalHistoryPage({ params }: { params: Params }) {
  ...
  useEffect(() => {
    const loadNames = async () => {
      try {
        const [dealers, sellers] = await Promise.all([fetchAllDealers(), fetchAllSellers()]);
  ...
  const handleSaveProposalData = useCallback(async () => {
    ...
    try {
      ...
      setEditModalState((previous) => ({ ...previous, open: false }));
      form.resetFields();
      messageApi.success("Ficha atualizada: Dados da ficha salvos com sucesso.");
  ...
  }, [form, metadata, proposal, reloadTimeline, messageApi]);
  ...
  const performStatusUpdate = useCallback(
    async (nextStatus: ProposalStatus, contractData?: {
      ...
      try {
        ...
        setProposal(updated);
        setNoteDraft(updated.notes ?? "");
        messageApi.success(`Status atualizado: A ficha agora esta ${statusLabel[nextStatus] ?? nextStatus}.`);
        await reloadTimeline();
      } catch (err) {
        const message =
  ...
  const handleSaveMessage = async () => {
    ...
    try {
      ...
      messageApi.success("Mensagem salva: Atualizamos a mensagem da analise.");
      await reloadTimeline();
    } catch (err) {
      const message =
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <Space wrap>
              <Button
                ...
                disabled={!proposal || isSavingProposalData || !canChangeProposalStatus}
              >
